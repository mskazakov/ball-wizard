...
/**
  ...
  // --- Ульта (день 1 недели 3) ---
  /**
   * До какого state.time.now ульта на кулдауне.
   * Игрок может кастовать ульту когда state.time.now >= ultaReadyAt.
   * Стартовое значение — 0 (готова сразу с начала рана).
   */
  ultaReadyAt: number;
...
/**
 * Снаряд ульты Ball-istics — пробивной выстрел в точку клика.
 * Летит по прямой, пробивает всех врагов на линии (каждого ровно один раз),
 * удаляется при выходе за арену. Отдельный тип от Projectile (автошары):
 *   - другая логика коллизий (не удаляется при попадании)
 *   - другой визуал (день 2)
 *   - не подвержен бунам автошар (More damage и т.п. — пока не задача дня 1)
 *
 * hitEnemies — Set врагов, которым этот снаряд уже нанёс урон. Защищает
 * от двойного попадания если на следующем кадре снаряд всё ещё пересекает
 * того же врага. Объекты Enemy сравниваются по ссылке — это работает,
 * пока враг жив (умерший враг будет удалён из state.enemies в cleanupDead,
 * и снаряд просто перестанет его "видеть" в проверке коллизий).
 */
export interface UltaProjectile {
  position: Vec2;
  velocity: Vec2;
  radius: number;
  damage: number;
...
export interface BoonDefinition {
  ...
  description: string;
  /**
   * Аббревиатура из 2-3 букв для иконки буна в HUD (левый низ экрана).
   * Например: 'MD' для More damage, 'FH' для Fast hands.
   */
  shortLabel: string;
  /**
   * Цвет плашки иконки буна в HUD. CSS-цвет (hex или rgb).
   * Используется как фон для иконки, белый текст поверх.
   */
  color: string;
  /**
...
export interface GameState {
  ...
  /** Снаряды ульты Ball-istics (пробивные). Отдельно от автошар — другая логика. */
  ultaProjectiles: UltaProjectile[];
